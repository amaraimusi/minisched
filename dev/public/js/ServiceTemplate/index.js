...
$(()=>{
	...
	rowExchange = new RowExchange('main_tbl', data, null, (param)=>{
		...
		let auto_save_url = 'service_template/auto_save';
	...
	pwms = new ProcessWithMultiSelection({
			tbl_slt:'main_tbl',
			id_slt:'js_pwms_id',
			ajax_url:'service_template/ajax_pwms',
...
function disabledBtn(btnElm, action_flg){
	...
	let url = 'service_template/disabled'; // Ajax通信先URL
...
function destroyBtn(btnElm){
	...
	let json_str = JSON.stringify(data);//データをJSON文字列にする。
	let url = 'service_template/destroy'; // Ajax通信先URL
...
/**
 * SPA型・詳細 ボタン押下時の処理  SPA型詳細画面を開く
 */
function clickDetailBtn(btn){
	...
	let ent = crudBase.getEntityByRowIndex(row_index);
...
function regAction(){
	...
	// SPA型・登録アクション
	crudBase.regAction(ent,'service_template/reg_action', {
		'callback': (param)=>{
			// DB登録後のコールバック処理内容
...
function clearA(){
	...
	location.href = 'service_template?clear=1';
}
